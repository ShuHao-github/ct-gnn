...
class SAGEMessenger(object):
    ...
    def __init__(self, embedding_size_deg,
        ...
        self.gcn = GCN(inp=self.embedding_size_deg, hid=(self.embedding_size_deg, self.n*self.embedding_size_deg),
                       out=(self.embedding_size_deg, self.embedding_size_deg), dropout=self.dropout_rate)

        self.gcn1 = GCN(inp=2*self.embedding_size_deg, hid=(2*self.embedding_size_deg, 2*self.embedding_size_deg),
                       out=(self.embedding_size_deg, self.embedding_size_deg), dropout=self.dropout_rate)
        ...
        self.fnn_out = FNN(hidden_layer_sizes=[self.n*self.embedding_size_deg],
                                  inp_size=self.n*self.embedding_size_deg,
                                  out_size=self.embedding_transformation_deg,
                                  name='self_transform')

        self.fnn_out2 = FNN(hidden_layer_sizes=[2*self.embedding_size_deg],
                                  inp_size=2*self.embedding_size_deg,
                                  out_size=2*self.embedding_transformation_deg,
                                  name='self_transform')
    ...
    def build(self, G, E, bs=1):
        ...
        ###gcntf
            ...
            self.Adjacency_matrix = self.get_n_hop_adj(self.Adjacency_matrix, self.num_hop, weighted=None)

            self.self_transforms = []
            self.intras = []
            for i in range(self.Adjacency_matrix.shape[0]):
                layer = self.Adjacency_matrix[i]
                a = self.gcn.build(self.self_transform, tf.cast(layer, tf.float32))
                # a = self.gat.build(self.self_transform, layer)

                # norm = RMSNorm(2*self.embedding_size_deg)
                # self.mamba = MambaBlock(num_nodes, 2*self.embedding_size_deg, 1)
                # a = norm(a)
                # a = self.mamba.build(a, layer)
                ...
                if self.type == 'CROSS':
                    # print("CROSS")
                    if i < 100:
                        mean_values = tf.expand_dims(tf.reduce_mean(a, axis=0), axis=0)
                        b = tf.expand_dims(tf.concat([mean_values, a], axis=0), axis=0)
            ...
            if self.type == 'CROSS':
                ...
                self.self_transform = self.fnn_out.build(a2)
                # a2 = self.fnn_out.build(a2)
        ...
        ####transformer new
            ...
            # self.self_transform = self.self_transform + a2
        ...
        # print("self_transform", self.self_transform)
           ...
           #######################################################################
            ...
            # self.self_transform = norm(self.self_transform)
            #
            # self.mamba = MambaBlock(int(np.size(self.Adjacency_matrix[0])), self.embedding_size_deg, 3)
